...
char help[] = {"[Master modes]\n"
	" c - console, type in commands\n"
		...
		"e - execute, load db, only in select/create mode\n "
...
void NcursesOther(Connection *conn, const char *file)
{
	...
	do {
		NcursesInfo("Mode: Select/Create");
		/* debug
		   getyx(body, gety, getx);
		...
		switch(input) {
			...
			case 'e':
				...
				NcursesReset("Database has been successfully loaded, exit this mode");
				NcursesReload();
				break;
